...
async function publishTweet(tweetText, options = {}) {
  // Caminho 1: OAuth 2.0 user token (mais novo, auto-renovado pelo workflow)
  const oauth2Token = process.env.TWITTER_OAUTH2_ACCESS_TOKEN;
  if (oauth2Token) {
    const r2 = await new Promise((resolve) => {
      const payload = JSON.stringify({ text: String(tweetText).slice(0, 280) });
      const req = https.request({
        hostname: 'api.twitter.com',
        path: '/2/tweets',
        method: 'POST',
        ...
        res.on('end', () => {
          try {
            const parsed = JSON.parse(body);
            if (res.statusCode === 201 && parsed.data && parsed.data.id) {
              resolve({ published: true, tweet_id: parsed.data.id, statusCode: res.statusCode });
            } else {
              resolve({ published: false, queued: false, statusCode: res.statusCode, error: (parsed.detail || parsed.title || body).slice(0, 180) });
            }
          } catch (e) {
            resolve({ published: false, queued: false, statusCode: res.statusCode, error: body.slice(0, 180) });
          }
        });
      });
      req.on('error', (e) => resolve({ published: false, queued: false, statusCode: 0, error: String(e) }));
      req.write(payload);
      req.end();
    });
    if (r2.published) return r2;
    // se falhou por token expirado/sem crédito, cai para o OAuth1 abaixo
    global.__lastOAuth2Attempt = r2;
  }

  // Caminho 2: OAuth 1.0a (método clássico)
        ...
        try {
          ...
          if (res.statusCode === 201 || res.statusCode === 200) {
            ...
            resolve({
              ...
              statusCode: res.statusCode,
              response: parsed,
              message: `Tweet queued in self-healing pipeline (HTTP ${res.statusCode}: ${parsed.title || parsed.detail || 'motivo desconhecido'})`
            });
          }
        } catch (e) {
